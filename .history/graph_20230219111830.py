...
class Vertex(object):
    ...
    def neighbourhood(self):
        """
        Returns the neighbourhood of the vertex i.e. number of nodes with specific label (color)
        """
        d = {}
        for w in self.neighbours:
            l = w.label
            count = d.get(l, 0)
            count += 1
            d[l] = count
...
class Graph(object):
    ...
    def fastPr(self):
        partitionNr = 0
        for v in self.vertices:
            partitionNr = max(partitionNr, v.label)
        partitionNr += 1
        partitions = [None] * partitionNr
        inQ = [False] * partitionNr
        queue = deque()
        for i in range(partitionNr):
            partitions[i] = set()
            inQ[i] = True
        queue.append(partitionNr-1)
        for v in self.vertices:
            partitions[v.label].add(v)
        ...
        while len(queue) > 0:
            ...
            for v in p:
                for w in v.neighbours:
                    involvedPs[w.label] = involvedPs.get(w.label, set())
                    involvedPs[w.label].add(w)
                    labelCount[w] = labelCount.get(w, 0) + 1

            for label in involvedPs.keys():
                relevantP = partitions[label]
                initialSize = len(relevantP)
                freq = {}

                for v in involvedPs[label]:
                    ...
                    relevantP.remove(v)
                    if len(relevantP) == 0:
                        relevantP = partitions[label] = freq[f]

                if len(relevantP) == initialSize:
                    continue
                ...
                for p in freq.values():
                    if p is relevantP:
                        continue
                    if len(p) < len(smallestP):
                        smallestP = p
                    newPLabel = len(partitions)
                    partitions.append(p)
                    for v in p:
                        v.label = newPLabel
                ...
                if inQ[label]:
                    ...
                    for p in freq.values():
                        ...
                        inQ.append(False)
                    smallestLabel = next(iter(smallestP)).label
                    inQ[smallestLabel] = True
                    queue.append(smallestLabel)

        return partitions

    def evenCheck(self, partition):
        evenCount = 0
        for v in partition:
            if v.initialGraph:
                evenCount += 1
    ...
    def partitionRefinements(self):
        """
        Split the graph to partitions
        """
        partitionNr = 0
        for v in self.vertices:
            partitionNr = max(partitionNr, v.label)
        ...
        while updated:
            updated = False
            for p in partitions:
                # If the color has only one vertex, no need to update
                if len(p) == 1:
                    continue
                # This represents the neighbourhood attribute of a vertices in the partition, chosen by the first vertex
                partitionNeighbourhood = next(iter(p)).neighbourhood
                # The dictionary that maps the neighbourhood attribute to new partition when created
                neighbourhoods = dict()
                # The copy makes removal when iterating possible
                copyPartition = list(p)
                for v in copyPartition:
                    # The color neighbourhood attribute is the same as its partition neighbourhood attribute,
                    # so no updates needed
                    if v.neighbourhood == partitionNeighbourhood:
                        continue
                    # Otherwise remove it from current partition
                    p.remove(v)
                    updated = True
                    vNeighbourhood = frozenset(v.neighbourhood)
                    # If the attribute is not in any available attributes create a new partition for that
                    if vNeighbourhood not in neighbourhoods:
                        # Add new attribute to the dict if it is not already in the dict
                        v.newLabel = partitionNr
                        neighbourhoods[vNeighbourhood] = {v}
                        partitions.append(neighbourhoods[vNeighbourhood])
                        partitionNr += 1
                    # Else add and set the existing set
                    else:
                        # Set the label to the partition label (first vertex)
                        pV = neighbourhoods[vNeighbourhood]
                        v.newLabel = next(iter(pV)).newLabel
                        pV.add(v)
                for v in copyPartition:
                    v.label = v.newLabel
        return partitions
    ...
    def isTree(self):
        ...
        for v in self.vertices:
            v.visited = False
        return connectedN - 1 == len(self.edges) and len(self.vertices) == connectedN

    def recEncode(self, node):
        node.visited = True
        codes = []
        for v in node.neighbours:
            if v.visited:
                continue
            codes.append(self.recEncode(v))
    ...
    def treeCenter(self):
        ...
        cur = headDiameter
        diameter.append(cur)
        while cur != tailDiameter:
            cur = cur.parent
            diameter.append(cur)

        mid = int(len(diameter) / 2)
        centers = [diameter[mid]]
        if len(diameter) % 2 == 0:
            centers.append(diameter[mid - 1])
        return centers

    def formTwinGroup(self):
        ...
        for v in self.vertices:
            ...
            if nb not in dic:
                ...
                v.group = dic[nb]
                v.group.twinType = 1
            groupCount[v.group] = groupCount.get(v.group, 0) + 1
        for v in self.vertices:
            groupSize = groupCount[v.group]
            v.groupSize = groupSize
        for i in groupCount.values():
