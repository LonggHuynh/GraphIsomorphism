...
def order(generators):
    ...
    generators = Stabilizer(generators, el)
    return len(orbit) * order(generators)


def isInGenerator(f, generators):
    if len(generators) < 2:
        return f in generators

    el = FindNonTrivialOrbit(generators)
    orbit, transversal = Orbit(generators, el, returntransversal=True)
    beta = f.P[el]
    ...
    for i in range(len(orbit)):
        if beta == orbit[i]:
            uB = transversal[i]
            s = Stabilizer(generators, el)
            return isInGenerator(-uB * f, s)
    return False
